...
class Build : NukeBuild
{
    ...
    /// <summary>
    ///     Gets the DotNet solution.
    /// </summary>
    [Solution]
    readonly Solution Solution = null!;

    /// <summary>
    ///     Gets the absolute path for nuget package output.
    /// </summary>
    static AbsolutePath PackagesDirectory => RootDirectory / "packages";

    /// <summary>
    ///     Gets the absolute path for the project's source code.
    /// </summary>
    static AbsolutePath SourceDirectory => RootDirectory / "src" / "code";

    /// <summary>
    ///     Gets the absolute path for the project's test folder.
    /// </summary>
    static AbsolutePath TestsDirectory => RootDirectory / "src" / "tests";

    /// <summary>
    ///     Gets a target that runs DotNet Build for all build configurations.
    /// </summary>
    Target BuildAll => targetDefinition => targetDefinition.DependsOn(BuildDebug, BuildRelease);

    /// <summary>
    ///     Gets a target that runs DotNet Build for all build configurations after running all dependencies.
    /// </summary>
    ...
    /// <summary>
    ///     Gets a target that runs DotNet Build for the Release configuration after running all dependencies.
    /// </summary>
    Target BuildReleaseWithDependencies => targetDefinition => targetDefinition.DependsOn(CleanRelease, Restore, BuildRelease);

    /// <summary>
    ///     Gets a target that runs DotNet Clean for all configurations, removes all bin and object folders, and ensures the package output folder is clean.
    /// </summary>
    Target CleanAll => targetDefinition => targetDefinition.DependsOn(CleanDebug, CleanRelease, CleanOutput, CleanPackages);

    /// <summary>
    ///     Gets a target that runs DotNet Clean for the debug configuration.
    /// </summary>
    Target CleanDebug => targetDefinition => targetDefinition.Executes(StandardClean(Configuration.Debug));

    /// <summary>
    ///     Gets a target that removes all bin and object folders.
    /// </summary>
    Target CleanOutput =>
    ...
    /// <summary>
    ///     Gets a target that ensures the package output folder is clean.
    /// </summary>
    ...
    /// <summary>
    ///     Gets a target that runs DotNet Clean for the release configuration.
    /// </summary>
    Target CleanRelease => targetDefinition => targetDefinition.Executes(StandardClean(Configuration.Release));

    /// <summary>
    ///     Gets a target that runs the default build.
    /// </summary>
    ...
    /// <summary>
    ///     Gets a target that runs DotNet Pack for the release configuration.
    /// </summary>
    Target Pack =>
        targetDefinition => targetDefinition.Executes(
            () =>
    ...
    /// <summary>
    ///     Gets a target that restores package dependencies.
    /// </summary>
    ...
    /// <summary>
    ///     Gets a target that runs DotNet Test for all build configurations.
    /// </summary>
    Target TestAll => targetDefinition => targetDefinition.DependsOn(TestDebug, TestRelease);

    /// <summary>
    ///     Gets a target that runs DotNet Test for all build configurations after running all dependencies.
    /// </summary>
    ...
    /// <summary>
    ///     Gets a target that runs DotNet Test for the debug configuration.
    /// </summary>
    ...
    /// <summary>
    ///     Gets a target that runs DotNet Test for the debug configuration after running all dependencies.
    /// </summary>
    Target TestDebugWithDependencies => targetDefinition => targetDefinition.DependsOn(CleanDebug, Restore, BuildDebug, TestDebug);

    /// <summary>
    ///     Gets a target that runs DotNet Test for the release configuration.
    /// </summary>
    ...
    /// <summary>
    ///     Gets a target that runs DotNet test for the release configuration after running all dependencies.
    /// </summary>
    ...
    /// <summary>
    ///     Creates an action that performs the project's standard DotNet Build task.
    /// </summary>
    /// <param name="configuration">
    ///     The build configuration to use.
    /// </param>
    /// <returns>
    ///     An action that performs the project's standard DotNet Build task.
    /// </returns>
    ...
    /// <summary>
    ///     Creates an action that performs the project's standard DotNet Clean task.
    /// </summary>
    /// <param name="configuration">
    ///     The build configuration to use.
    /// </param>
    /// <returns>
    ///     An action that performs the project's standard DotNet Clean task.
    /// </returns>
    ...
    /// <summary>
    ///     Creates an action that performs the project's standard DotNet Restore task.
    /// </summary>
    /// <returns>
    ///     An action that performs the project's standard DotNet Restore task.
    /// </returns>
