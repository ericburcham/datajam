...
internal sealed class RepresentationRepository
{
        ...
        where T : class
    {
        ...
        var itemTypes = new List<Type>(itemType.GetInterfaces())
        {
            itemType
        };
    ...
    private Representation CreateChildObjectRepresentation(
        ...
        object parent,
        Action removeAction,
        Func<object, object, object> getterFunc)
    {
        if (Exists(item))
        {
        ...
        {
            var objectRepresentation = new Representation(item, AddRelatedObjects(item))
            {
    ...
    private void UpdateExistingRepresentations(Representation representation)
    {
        ...
        foreach (var data in representation.RelatedEntities.Where(x => typesCurrentlyStored.Contains(x.Entity.GetType())))
        {
            ...
            var propertiesThatReferToRepresentation =
                data.Entity.GetType()
                    .GetProperties()
                    .Where(x => x.PropertyType == entityType || x.PropertyType.IsAssignableFrom(collectionType));
